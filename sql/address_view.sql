SELECT 
    m.Connect_ID,
    CAST(m.ts_user_profile_updated AS STRING) AS ts_user_profile_updated,
    m.ts_address_delivered,
    m.address_src_question_cid,
    m.address_nickname,
    m.address_source,
    CAST(m.historical_order AS INT64) AS historical_order,
    CAST(m.address_line_1  AS STRING) AS address_line_1,
    CAST(m.address_line_2  AS STRING) AS address_line_2,
    CAST(m.street_num      AS STRING) AS street_num,
    CAST(m.street_name     AS STRING) AS street_name,
    CAST(m.apartment_num   AS STRING) AS apartment_num,
    CAST(m.city            AS STRING) AS city,
    CAST(m.state           AS STRING) AS state,
    CAST(m.zip_code        AS STRING) AS zip_code,
    CAST(m.country         AS STRING) AS country,
    CAST(m.cross_street_1  AS STRING) AS cross_street_1,
    CAST(m.cross_street_2  AS STRING) AS cross_street_2
FROM (

    SELECT
        CAST(Connect_ID AS STRING) AS Connect_ID,
        CAST(NULL AS TIMESTAMP) AS ts_user_profile_updated,
        CURRENT_TIMESTAMP() AS ts_address_delivered,
        '121490150' AS address_src_question_cid,
        'home_address_01' AS address_nickname,
        'module4' AS address_source,
        NULL AS historical_order,  
...
JOIN @raw_participants p ON CAST(m.Connect_ID AS STRING) = CAST(p.Connect_ID AS STRING)
WHERE
    p.d_821247024 = 197316935 -- Verification status = verified
    AND p.d_831041022 = 104430631 -- Data destruction requested = no
    AND p.d_663265240 = 231311385 -- Module 4 is complete
    AND (
        (m.street_num IS NOT NULL AND m.street_num != '') OR
        (m.street_name IS NOT NULL AND m.street_name != '') OR
        (m.apartment_num IS NOT NULL AND m.apartment_num != '') OR
        (m.city IS NOT NULL AND m.city != '') OR
        (m.state IS NOT NULL AND m.state != '') OR
        (m.zip_code IS NOT NULL AND m.zip_code != '') OR
        (m.country IS NOT NULL AND m.country != '') OR
        (m.cross_street_1 IS NOT NULL AND m.cross_street_1 != '') OR
        (m.cross_street_2 IS NOT NULL AND m.cross_street_2 != '')
    )